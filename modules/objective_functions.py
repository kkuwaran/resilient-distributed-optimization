...
from utilities.logistic_regression import Logistic_Regression


def change_labels(y_true, prob=0.0, n_class=2):
    '''Change labels with given probability 'prob' to a class chosen at uniformly random
    ========== Inputs ==========
    y_true - np.ndarray: original labels
    prob - float in interval [0, 1]: probability of flipping labels
    n_class - int: number of classes
    ========== Outputs ==========
    y_out - np.ndarray: flipped labels
    '''

    # check inputs requirements
    assert isinstance(y_true, np.ndarray), "incorrect labels type"
    assert np.all(np.logical_and(0 <= y_true, y_true <= (n_class - 1))), "incorrect labels"
    assert 0.0 <= prob <= 1.0, "incorrect probability"
    assert isinstance(n_class, int), "incorrect n_class type"

    # calculate (uniformly) random changing labels
    y_temp = np.random.randint(n_class - 1, size=y_true.shape)
    y_new = y_temp + (y_temp >= y_true)  # +1 if y_temp >= y_true
    change_flag = np.random.rand(*y_true.shape) < prob
    y_out = (change_flag * y_new) + ((1 - change_flag) * y_true)
    return y_out


class DecentralizedQuadratic:
    '''Decentralized Quadratic Functions Simulation for Decentralized Optimization'''
    ...
    def check_and_display(self):
        ...
        print(f"Q Matrices for first {self.n_displays} nodes: \n{self.Qs[:self.n_displays]}")
        print(f"b Vectors for first {self.n_displays} nodes: \n{self.bs[:self.n_displays]}")
        print(f"Local Minimizer for first {self.n_displays} nodes: \n{self.minimizers[:self.n_displays]}")
...
class DecentralizedBankNotes:
    '''Link: https://archive.ics.uci.edu/dataset/267/banknote+authentication
    INFO: Data were extracted from images that were taken for the evaluation of an authentication procedure for banknotes
    DATASET: 1372 instances, 4 attributes (variance, skewness, curtosis, entropy), 2 classes (0: genuine, 1: forged)
    '''
    ...
    def __init__(self, n_nodes, bias_flag=False, n_rand=None, display_flag=False):
    ...
    def dataset_initialization(self):
        ...
        train, test = self.banknotes_initialization(self.N_TRAINS)
        ...
        if self.display_flag:
            print(f"\n===== {self.DATASET_NAME} Dataset Information =====")
            print(f"Total Number of Data: {n_data}")
            print(f"Number of Features: {n_features}")
            print(f"(Centralized) Training Shape: X-{X_train.shape}; y-{y_train.shape}")
            print(f"Test Shape: X-{test[0].shape}; y-{test[1].shape}")
        ...
        if self.display_flag:
            ...
            print(f"Number of Samples for each Agent: {n_samples_list}")
            print(f"(Decentralized) Training Shape: X-{X_train_dec[0].shape}; y-{y_train_dec[0].shape}")
    ...
    def banknotes_initialization(self, n_trains):
        ...
        try:
            ...
            try:
                ...
                raise FileNotFoundError("BankNotes Dataset not found")
        
        # display pairplot of dataset
        if self.display_flag:
    ...
    def regularizer_calculation(self, dataset, val_ratio=0.2):
        ...
        if self.display_flag:
            ...
            print(f"(Non-Byzantine) Training Shape: X-{X_train.shape}; y-{y_train.shape}")
            print(f"(Non-Byzantine) Validation Shape: X-{X_val.shape}; y-{y_val.shape}")
        ...
        if self.display_flag:
            ...
            print(f"Optimal Regularizer: {regularizers[opt_index]:.3f}")
            ...
            print(f"(Non-Byzantine) Optimal Parameters: \n{minimizer}")
            print(f"(Non-Byzantine) Logistic Loss at Optimal: {opt_value:.4f}")
            print(f"(Non-Byzantine) Gradient Norm at Optimal: {LA.norm(gradient):.8f}")
    ...
    def local_minimizer_calculation(self):
        ...
        if self.display_flag:
            ...
            for i in range(self.n_nodes):
                ...
                print(f"Optimal Parameters of {i+1}-th Local Model: {minimizers[i]}")
                print(f"   Logistic Loss of Node {i+1}: {self.n_nodes * loss:.4f}")
                print(f"   Gradient Norm of Node {i+1}: {self.n_nodes * LA.norm(grad):.8f}")
    ...
    def global_optimal_calculation(self, indices=None, attribute_flag=True):
        ...
        if self.display_flag:
            print("\n===== Global Minimizer and Function Value =====")
            print(f"Minimizer of indices Nodes Sum: {global_minimizer}")
            print(f"Optimal Value: {global_opt_value:.4f}")
            print(f"Gradient Norm at Optimal: {LA.norm(gradient):.8f}")
        ...
        if attribute_flag:
            ...
            if self.display_flag:
                ...
                print(f"Benchmark Accuracy (train): {benchmark_train_acc:.2f}")
                print(f"Benchmark Accuracy (test): {benchmark_test_acc:.2f}")
    ...
    # ============================== [Utility] Adversarial Attack Functions ==============================

    def attack_labels(self, indices, prob=0.0):
        '''Attack training labels for nodes within given indices (with given probability 'prob')
        make changes to self.y_train_dec[index] for each index in 'indices'
        ========== Inputs ==========
        indices - list of len=n_points: list of node indices being attacked
        prob - float in interval [0, 1]: probability of flipping labels
        '''
        ...
        # flip labels for y_train_dec[index]
        for index in indices:
            y_true = self.y_train_dec[index]  # shape = (samples, 1)
            y_flipped = change_labels(y_true, prob=prob, n_class=self.N_CLASS)
            self.y_train_dec[index] = y_flipped
    ...
    def model_evaluation(self, dataset, state, names=['', '']):
        ...
        if self.display_flag:
            ...
            print(f"Accuracy of {state_name} ({mode_name}): {accuracy:.2f}")
